...
function deploy_block_vpc_csi_driver {
   # Check if add-on already exists
   ibmcloud ks cluster-addons --cluster $BLOCK_VPC_CLUSTER | grep vpc-block-csi-driver
   retCode=$?
   if [[ $retCode != 0 ]] ; then
       echo "vpc-block-csi-driver add-on need to be enabled"
       ibmcloud ks cluster-addon-enable vpc-block-csi-driver --cluster $BLOCK_VPC_CLUSTER
       echo "Exit status of driver deployment command is  $?"
   else
       echo "vpc-block-csi-driver already exists"
   fi
   check_statefulset "ibm-vpc-block-csi-controller"
   check_daemonset_state "ibm-vpc-block-csi-node"
}
...
function create_sc {
    kubectl create -f ./deploy/kubernetes/storageclass/ibmc-vpc-block-5iopsTier-StorageClass.yaml
    kubectl create -f ./deploy/kubernetes/storageclass/ibmc-vpc-block-generalpurpose-StorageClass.yaml
    kubectl create -f ./deploy/kubernetes/storageclass/ibmc-vpc-block-custom-StorageClass.yaml
}
